# Runs the calculation and plotting Kapitza conductivity
# Запуск вычисления и рисования графиков проводимости Капицы
...
def run_kapitza_sweep_make_plots_and_excel(
    hk_fn,
    pairs,
    stack_pair_names=("Al | Si3N4", "Si3N4 | SiO2", "SiO2 | Si"),
    ...
    def _hk_safe(A, B, T, sigma):
        h = hk_fn(A, B, float(T), sigma=float(sigma), Lcorr=float(Lcorr),
                  theta_min_deg=float(theta_min_deg),
                  n_theta=n_theta, nphi=nphi, n_omega=n_omega, symmetric=True)
    ...
    for sigma in sigma_list:
        ...
        per_sigma = {}
        for name, matA, matB in pairs:
            h_arr, R_arr = _compute_pair_table(matA, matB, sigma)
            per_sigma[name] = {"h": h_arr, "R": R_arr}
        # stack
        R_stack = None
        for pname in stack_pair_names:
            arr = per_sigma[pname]["R"]
            R_stack = arr if R_stack is None else (R_stack + arr)
        per_sigma["stack"] = {"h": np.where(np.isfinite(R_stack)&(R_stack>0.0), 1.0/R_stack, 0.0),
                              "R": R_stack}
        by_sigma[key] = per_sigma
    ...
    with pd.ExcelWriter(xlsx_path, engine="xlsxwriter") as writer:
        for sigma in sigma_list:
            key = f"{int(round(sigma*1e9))}nm"
            rows = []
            for pair_name, val in by_sigma[key].items():
                for T, hK, R in zip(T_list, val["h"], val["R"]):
                    rows.append({"Interface": pair_name, "T_K": int(T),
            ...
            ws = writer.sheets[sheet]; wb = writer.book
            fmt_sci = wb.add_format({"num_format": "0.00E+00"})
            ws.freeze_panes(1, 0)
            ws.set_column("A:A", 22); ws.set_column("B:B", 8); ws.set_column("C:D", 16, fmt_sci)

    # Two grouped figures only
    T = np.array(T_list, float)
    per_ifc_dict = {}
    for name, _, _ in pairs:
        per_ifc_dict[name] = {key: by_sigma[key][name] for key in by_sigma.keys()}
    per_ifc_dict["stack"] = {key: by_sigma[key]["stack"] for key in by_sigma.keys()}
    plot_group_all_interfaces(T, per_ifc_dict, sigma_list, colors_map, out)
...
if __name__ == "__main__":
    pairs = default_pairs()
    stack_order = tuple(name for name, *_ in pairs)  # order as in pairs
    ...
    run_kapitza_sweep_make_plots_and_excel(
        ...
        pairs=pairs,
        stack_pair_names=stack_order,
        T_list=(50, 100, 150, 200, 250, 300),
        sigma_list=(0e-9, 1e-9, 5e-9, 10e-9, 16e-9),
        Lcorr=10e-9,
        theta_min_deg=0.0,
        n_theta=801, nphi=241, n_omega=1601,
        outdir="plots",
        excel_filename=None
    )
